...
public class ParkingDataBase {
	public static Connection setConnection() {
		...
		try {
			...
			con = DriverManager.getConnection("jdbc:mysql://localhost:3306/eparking?useSSL=false", "root", "root");
	...
	public void InsertCarDetail(String TableName, int id, String Reg, String Pos) {
		...
		try {
			Connection con = setConnection();
			Statement st = con.createStatement();
			st.executeUpdate("INSERT into " + TableName + "Detail(ID,RegNo,Position,ParkingDate,TimeIn)VALUES(" + id
					+ ",'" + Reg + "','" + Pos + "','" + date + "','" + Time + "')");
			st.executeUpdate(
					"UPDATE " + TableName + "Position set RegNo = '" + Reg + "' Where Position = '" + Pos + "'");
	...
	public void UpdateCarPosition(String TableName, String pos) {
		try {
			Connection con = setConnection();
			Statement st = con.createStatement();
			st.executeUpdate("UPDATE " + TableName + " set RegNo = '1' Where Position = '" + pos + "'");
			st.executeUpdate("UPDATE " + TableName + " set ID = 1 Where Position = '" + pos + "'");
